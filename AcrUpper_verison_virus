...
parser = argparse.ArgumentParser(description='## Must have Known_Acr.faa file and "hmmpress hmmdb_HTH_domain" output file and the assembly_summary file present in the directory of this script!!'
                                             '\n## This script will find Acr homologs with diamond, Find potential Acr-Aca loci and find perform a hmmscan with the 65 published HTH domain againest those loci.'
                                             '\n## Output of the script: Acr homologs fasta file; Potential Acr-Aca loci file; Candidate Aca fasta file; Diamond output file hmmscan output file'
                                             '\n## look for file "!!!!! Potential Acr-Aca loci found and saved in %s"; "$$$$$ Aca candidate found and saved in file %s" for final output results')
parser.add_argument('-g','--GCF_folder',help="folder of GCF containg faa gff files")
parser.add_argument('-s','--summary_file',default="NA",type=str,help='virus summary_file [default=NA]')
parser.add_argument('-o','--outputFolder',type=str,help="folder containing all output results of all ran GCFs")
...
def find_faa(GCF_directory):
    ...
    faa = subprocess.run(["find", GCF_directory, "-maxdepth", "15", "-name", "*_prodigal_out.faa"],stdout=subprocess.PIPE).stdout.decode('utf-8')
    faa = str(faa).strip()
    return faa

def find_gff(GCF_directory):
    ##modified for prodigal
    gff = subprocess.run(["find", GCF_directory, "-maxdepth", "15", "-name", "*_prodigal_out.gff"],stdout=subprocess.PIPE).stdout.decode('utf-8')
    gff = str(gff).strip()
    return gff
...
def viruse_info_make(assembly_dic,contig):
    contig=contig.replace("REF:","")
    #Not sure why it alwasys comes up to be REF in contig
    if contig in assembly_dic.keys():
        return assembly_dic[contig]
    if contig not in assembly_dic.keys():
        print("%s not in virus_info_summary or no summary file provided/available"%contig)
        return []

def result_check_list(list,assembly_dic):
    gene_strand=strand(list)
    gene_length=length(list)
    viruse_info=viruse_info_make(assembly_dic,contig(list))
    if "pseudo=true" not in list[-1]:ProID=proteinID(proteinInfo(list))
    elif "pseudo=true" in list[-1]:ProID="pseudo"
    if len(viruse_info) >0:
        # output result in format ProteinID; contig;gene strand; gene length；gene start;gene end, Ecosystem, Ecosystem_Category, Ecosystem_Type, Ecosystem_Subtype, Habitat; in a list
        check_result = [ProID, contig(list), gene_strand, gene_length, list[3], list[4],viruse_info[0],viruse_info[1],viruse_info[2],viruse_info[3],viruse_info[4]]
    elif len(viruse_info) ==0:
...
def loci_select_before_diamond(file_dic,assembly_dic):
    ###################################################################
    ...
    while key in file_dic.keys():
        ...
        if length(file_dic[key]) < 600:
            ...
            if key-1 in file_dic.keys() and strand(file_dic[key]) == strand(file_dic[key-1]) and contig(file_dic[key]) == contig(file_dic[key-1]) and length(file_dic[key-1]) < 600:
                ...
                lst_result_check.append(result_check_list(file_dic[key],assembly_dic))
                key=key+1
                ...
                #This is for result checking
                lst_result_check.append(result_check_list(file_dic[key],assembly_dic))
                lst_result_check.append(result_check_list(file_dic[key+1],assembly_dic))
...
def run_diamond(database_file,outputfile):
    ##Use this as dmond_out
    subprocess.Popen(['diamond','makedb','--in',database_file,'-d',str(outputfile)+"_diamond_db"]).wait()
    subprocess.Popen(['diamond','blastp','-d',str(outputfile)+"_diamond_db",'-f','6','--quiet','--more-sensitive','-q','Known_Acr.faa','-p','5','-o',str(outputfile)+'.dmnd_out']).wait()
...
def assembly_sum_make(summary_file):
    assembly_dic = {}
    assem = subprocess.Popen(["awk -F '\t' 'NR>1 {print $0}' %s"%summary_file],
                             shell=True, stdout=subprocess.PIPE)
    for line in assem.stdout:
        line = line.strip().decode('utf-8')
        line = line.split("\t")
        assembly_dic.setdefault(line[0], [line[4], line[5], line[6], line[7], line[8]])
    return assembly_dic
##########################################################################################

GCF_directory=args.GCF_folder
##This is the GCF file directory
output_folder=make_and_check_output_directory(args.outputFolder)
...
### Write the assembly information into a dictionary, changed according to virus assembly info
if "NA" not in summary_file:assembly_dic=assembly_sum_make(summary_file)
elif "NA" in summary_file:
    assembly_dic={}
...
##The GCF and assembly_dic is for result checking, specifically for making the species names
    loci_list,loci_list_result_check=loci_select_before_diamond(file_dic,assembly_dic)
