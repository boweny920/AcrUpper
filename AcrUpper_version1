# Developed by Bowen Yang Jun29 2020
import subprocess
import sys
...
parser.add_argument('-a','--assembly_sumfile',type=str,help='assembly summary file')
...
def find_faa(GCF_directory):
    faa=subprocess.run(["find",GCF_directory,"-maxdepth","15","-name","*protein.faa"],stdout=subprocess.PIPE).stdout.decode('utf-8')
    faa=str(faa).strip()
    return faa

def find_gff(GCF_directory):
    gff=subprocess.run(["find",GCF_directory,"-maxdepth","15","-name","*genomic.gff"],stdout=subprocess.PIPE).stdout.decode('utf-8')
    gff=str(gff).strip()
    return gff
...
def faa_file_wrote_diamond_special(one_acr_aca_locus,faa_file,newfile_name_dirctory,Acr_protein_NP_list):
    ##This function takes the proteins that are not Acrs and write them into a new faa file(this was the original plan, but writen the way as now, the new acas will include Acr-Acas also, so i prefer not to change it )
...
##Need to change to Refseq Bacteria Assembly_summary file###
assembly_dic={}
assem=subprocess.Popen(["awk 'NR>2 {print $0}' %s"%args.assembly_sumfile],shell=True,stdout=subprocess.PIPE)
...
##The GCF and assembly_dic is for result checking, specifically for making the species names
    ...
    if is_non_zero_file(diamond_outfile) is not False:
        ...
        ### For result checking, v[0] is the Protein ID, if v[0] is an Acr homolog, then append to list v the Acr type
